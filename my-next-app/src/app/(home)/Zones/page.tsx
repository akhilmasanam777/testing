...
export default function ZonesPage() {
    ...
    async function loadPackages() {
        const res = await fetch(
            "https://bnpapp.traxion.in/api/dropdowns/GetPackage?id=0&id2=0"
        );
        const data = await res.json();
    ...
    // ================= ADD =================
    function handleAdd() {
        setForm({ Id: 0, Name: "", Code: "", PackageId: "" });
        setOpen(true);
    }
...
const columns: (keyof Zone)[] =
    zones.length > 0
        ? (Object.keys(zones[0]).filter(
            (key) =>
                key !== "PackageId" &&
                key !== "ZoneId"
        ) as (keyof Zone)[])
    ...
    // const columns:(keyof Zone)[] = zones.length > 0
    //     ? Object.keys(zones[0]).filter(
    //         (key) => key !== "PackageId" && key !== "ZoneId"
    //     )
    //     : [];
...
const bVal =
    ...
    const exportCSV = () => {
        const csv = zones.map((row) => Object.values(row).join(",")).join("\n");
    ...
    const exportPDF = () => {
        const doc = new jsPDF();
        autoTable(doc, {
            head: [Object.keys(zones[0] || {})],
            body: zones.map((row) => Object.values(row)),
        });
                                ...
                                <TableRow key={row.ZoneId}>
                                    ...
                                    <TableCell >
                                        ...
                                        {permissions.CanDelete && (
                                            <button
                                                onClick={() => handleDelete(row.ZoneId)}
