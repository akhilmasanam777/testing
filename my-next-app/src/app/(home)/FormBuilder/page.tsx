...
function FormBuilder({
  ...
  const addBackendValueForField = async (field: Field, rawValue: string) => {
    ...
    try {
      ...
      if (vsId) {
        setValueSets((prev) =>
          prev.map((set) => (set.id === vsId ? { ...set, v: normalized.map((o) => o.name) } : set)),
        );
      }

      // Auto-select the newly created value on this field.
      const newOption = normalized.find((o) => (savedId && o.id === savedId) || o.name.trim().toLowerCase() === value.toLowerCase());
      if (newOption) {
        const selectedIds = new Set((field.selectedOptionIds ?? []).map(Number));
