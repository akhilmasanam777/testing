...
async function getPermissions(token: string): Promise<Required<Permissions>> {
  const res = await backendFetch(
    token,
    "/Web/permissions/page?controller=RowRequest&action=Index",
  );
...
function dropdownPath(type: string, searchParams: URLSearchParams) {
  const zoneId = searchParams.get("zoneId");
  const districtId = searchParams.get("districtId");

  switch (type) {
    case "zones":
      return "/api/dropdowns/GetZones";
    case "districts":
      return zoneId
        ? `/api/dropdowns/GetDistrictByZone?id=${encodeURIComponent(zoneId)}`
        : "/api/dropdowns/GetDistrict";
    case "blocks":
      return districtId
        ? `/api/dropdowns/GetMandalByDistrict?id=${encodeURIComponent(districtId)}`
        : "/api/dropdowns/GetMandal";
    case "appliedThrough":
      return "/api/dropdowns/GetLookUp?code=177";
    case "rowAuthority":
      return "/api/dropdowns/GetLookUp?code=156";
    case "approvedStatus":
      return "/api/dropdowns/GetLookUp?code=178";
    default:
      return "";
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    if (type) {
      const path = dropdownPath(type, searchParams);
      if (!path) {
    ...
    if (logId) {
      const res = await backendFetch(
        token,
        `/Web/invoicerow/getrowrequestlogbyid?Id=${encodeURIComponent(logId)}`,
      );
    ...
    if (id) {
      const res = await backendFetch(
        token,
        `/Web/invoicerow/getrowrequestbyid/${encodeURIComponent(id)}`,
      );
      return forwardResponse(res);
    }

    const res = await backendFetch(token, "/Web/invoicerow/getrowrequest");
    const data = await readPayload(res);
...
export async function POST(req: Request) {
  try {
    ...
    const res = await backendFetch(token, "/Web/invoicerow/saverowrequest", {
      method: "POST",
      body: formData,
    });
...
export async function DELETE(req: Request) {
  try {
    ...
    const res = await backendFetch(
      token,
      `/Web/invoicerow/deleterowrequest/${encodeURIComponent(id)}`,
      { method: "GET" },
    );
